...
const url =
	"https://qubic.github.io/integration/Partners/swagger/qubic-http.swagger.json";
...
https.get(url, (res) => {
	...
	res.on("end", () => {
		...
		fs.writeFileSync("src/gen/http-types.ts", out);
		console.log(
			`wrote ${Object.keys(defs).length} types to src/gen/http-types.ts`,
		);
	});
});
